...
Даты берутся из `.env` (`START_DATE` / `END_DATE`).

Перед вставкой таблица очищается, так что повторный запуск не размножает строки. В Sheets каждый раз добавляется новая строка сводки.
